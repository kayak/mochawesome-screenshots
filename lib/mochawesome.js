...
function Mochawesome (runner, options) {
  ...
  runner.on('test', function (test) {
    if (config.multiReport) {
      test.reportName = test.parent.fullTitle().replace(/[^\w_\-]/g, '_').replace(/_+/g, '_').toLowerCase();
    }
  ...
  runner.on('pass', function (test) {
    ...
    if (config.passedScreenshot) {
      allPromises.push(saveScreenshotToFile(config.reportScreenshotDir + '/' + test.scrFileName));
    }
  });
...
Mochawesome.getPromises = function() {
  return allPromises;
}
...
function cleanTest (test) {
  ...
  var err = test.err ? _.pick( test.err, ['name', 'message', 'expected', 'actual', 'stack', 'scr'] ) : test.err,
    scr = '<img src="./screenshots/' + (test.customScrFileName ? test.customScrFileName : test.scrFileName) + '">';
